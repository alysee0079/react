...
function ensureRootIsScheduled(root, currentTime) {
  // 前半部分: 判断是否需要注册新的调度
  // callbackNode 在 commitRootImpl 中被重置为 null
  ...
  if (existingCallbackNode !== null) {
    ...
    // 节流: 当前优先级和 root 已经存在的优先级相同, 如连续多次执行setState, 则无需注册新 task (继续沿用上一个优先级相同的task), 直接退出调用.
    ...
    // 防抖: 当前优先级和 root 已经存在的优先级不同, 则取消旧 task, 重新注册新 task.
    cancelCallback(existingCallbackNode);
