...
// fn: function () { return dispatchEventsForPlugins(domEventName, eventSystemFlags, nativeEvent, ancestorInst)}
function batchedEventUpdates(fn, a, b) {
  ...
  try {
    // 通过插件派发事件, fn: dispatchEventsForPlugins
...
function discreteUpdates(fn, a, b, c, d) {
  ...
  try {
    // discreteUpdatesImpl: batchedUpdates$1
...
// legacy 模式没有优先级的概念
var ImmediatePriority = 99; // 同步执行优先级
var UserBlockingPriority = 98; // 用户操作触发的更新优先级
var NormalPriority = 97; // 普通优先级, 例如数据请求后更新
var LowPriority = 96;
var IdlePriority = 95; // NoPriority is the absence of priority. Also React-only.

var NoPriority = 90; // 无优先级
...
function dispatchDiscreteEvent(domEventName, eventSystemFlags, container, nativeEvent) {
  // 处理之前未处理完的 DiscreteEvent 与 useEffect 回调
  {
...
function extractEvents$4(dispatchQueue, domEventName, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  // 匹配触发事件对应的 react 事件: click => onClick
...
function processDispatchQueueItemsInOrder(event, dispatchListeners, inCapturePhase) {
  ...
  if (inCapturePhase) {
    for (var i = dispatchListeners.length - 1; i >= 0; i--) {
      ...
      // 如果 event.isPropagationStopped() 返回 true 即为阻止事件冒泡
...
function scheduleSyncCallback(callback) {
  ...
  if (syncQueue === null) {
    ...
    // Scheduler_ImmediatePriority 最高优先级, 在下一次事件循环执行
    // immediateQueueCallbackNode: 储存异步更新任务
...
function flushSyncCallbackQueue() {
  // immediateQueueCallbackNode: 创建的异步更新任务
  if (immediateQueueCallbackNode !== null) {
    var node = immediateQueueCallbackNode;
    immediateQueueCallbackNode = null;
    // 清除执行函数, 既异步时不再执行此任务(跳过任务)
...
function enqueueUpdate(fiber, update) {
  // 将 update 对象并添加到 fiber.updateQueue.shared.pending
...
function processUpdateQueue(workInProgress, props, instance, renderLanes) {
  // 将 fiber.updateQueue.shared.pending 合并到 fiber.updateQueue.firstBaseUpdate 队列上
  ...
  // 本次更新之前遗留的 update(优先级低的 update)
  var firstBaseUpdate = queue.firstBaseUpdate;
  var lastBaseUpdate = queue.lastBaseUpdate; // Check if there are pending updates. If so, transfer them to the base queue.

  // 新产生的 updateQueue 环状列表
  var pendingQueue = queue.shared.pending; // 指向 lastUpdate
  ...
  if (pendingQueue !== null) {
    ...
    var lastPendingUpdate = pendingQueue; // 队尾 update
    var firstPendingUpdate = lastPendingUpdate.next; // 队首 update
    // 将队尾的 update next 指向空(解开 updateQueue 环状列表)
    lastPendingUpdate.next = null; // Append pending updates to base queue

    // 将产生的 update 添加到之前遗留的 updateQueue.lastBaseUpdate 队列后
    // 1.如果 lastBaseUpdate 不存在, 则 firstBaseUpdate 也不存在, 证明之前没遗留 update
    // 需要将新产生队首的 update 赋值为 firstBaseUpdate
    if (lastBaseUpdate === null) {
      firstBaseUpdate = firstPendingUpdate;
    } else {
      // 2.如果 lastBaseUpdate 存在, 则证明上次有遗留的 update, 需要将新产生的 队首的 update 放到之前队尾的 update 的后面,
      // 这时, 之前的 update 队列和新的 update 队列就链接在了一起
      lastBaseUpdate.next = firstPendingUpdate;
    }

    // 将 lastBaseUpdate 指向新的 update 的队尾, 此时完成环状链表
    ...
    // 在 current 上保存一份对应的 update(防止数据丢失)
    if (current !== null) {
      // This is always non-null on a ClassComponent or HostRoot
      var currentQueue = current.updateQueue;
  ...
  if (firstBaseUpdate !== null) {
    ...
    do {
      ...
      if (!isSubsetOfLanes(renderLanes, updateLane)) {
        ...
        // newFirstBaseUpdate, newLastBaseUpdate 会作为下次更新的 update
...
// 在fiber 树构造(初次创建)中的beginWork阶段, class 类型的组件初始化完成之后, this.updater对象如下(enqueueSetState)
...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function placeChild(newFiber, lastPlacedIndex, newIndex) {
    ...
    // current 存在, 说明是复用了 current 的 fiber 创建的
    if (current !== null) {
      // 获取 fiber 对应的 current 中的位置
      var oldIndex = current.index;

      // oldIndex 遍历到的可复用节点在 oldFiber 中的位置索引,
      // lastPlacedIndex 上一个匹配到的可复用的节点在 oldFiber 中的位置索引
      // 如果 oldIndex < lastPlacedIndex 说明本次更新的 fiber 节点向右移动了, lastPlacedIndex 不变
      if (oldIndex < lastPlacedIndex) {
        ...
        // 如果 oldIndex >= lastPlacedIndex, 说明本次更新的 fiber 节点位置没变, 更新 lastPlacedIndex 为 oldIndex
      ...
      // fiber 是重新创建的, 标记插入
      newFiber.flags = Placement;
      // current 不存在, 说明是完全重新创建的节点,  lastPlacedIndex 不变
      return lastPlacedIndex;
    }
  }
  ...
  function placeSingleChild(newFiber) {
    ...
    // 更新阶段, 且上次更新不存在 fiber, 本次更新存在 fiber, 需要做新增插入
  ...
  function updateElement(returnFiber, current, element, lanes) {
    ...
    // elementType 不一致或者 oldFiber 不存在, 创建全新的 fiber
  ...
  function updateSlot(returnFiber, oldFiber, newChild, lanes) {
    ...
    if (typeof newChild === 'string' || typeof newChild === 'number') {
      ...
      // we can continue to replace it without aborting even if it is not a text
      // node.
      // oldFiber 的 key 存在, 有可能是其他节点的 fiber(此场景可能是移动位置或新增删除), 跳出到第二次遍历去处理
      if (key !== null) {
        return null;
      }

      // 对应的 oldFiber 的 key 不存在, 则之前也是文本节点, 复用创建新的 fiber
        ...
        case REACT_ELEMENT_TYPE:
          {
            if (newChild.key === key) {
              ...
              // 判断 elementType 是否一致, 一致则复用 fiber, 不一致创建新 fiber
              return updateElement(returnFiber, oldFiber, newChild, lanes);
            } else {
              // key 不同, 返回 null(可能是位置移动或者删除), 跳出遍历, 到第二次遍历处理
              return null;
            }
          }
        ...
        case REACT_ELEMENT_TYPE:
          {
            ...
            // 匹配 elementType, 如果相同则复用 fiber, 否则创建新的 fiber
...
function cloneChildFibers(current, workInProgress) {
  ...
  // 创建子 fiber 节点, 如果 current 存在对应 fiber 节点, 进行复用
...
function renderWithHooks(current, workInProgress, Component, props, secondArg, nextRenderLanes) {
  ...
  // 执行函数组件, 获取返回值(组件的 ReactElement 树)
...
function mountReducer(reducer, initialArg, init) {
  // 创建 hook 并储存到 fiber.memoizedState, 或者拼接到当前处理的 hook 之后
  ...
  // dispatchAction 最终会执行 currentlyRenderingFiber$1.lastRenderedState(即传入得 reducer 或者默认的 basicStateReducer)
...
function updateReducer(reducer, initialArg, init) {
  ...
  if (pendingQueue !== null) {
    ...
    if (baseQueue !== null) {
      ...
      // baseQueue.next 之前指向 baseQueue 的队首, 修改指向为 pendingQueue 的队首
      baseQueue.next = pendingFirst;
      // pendingQueue.next 之前指向 pendingQueue 的队首, 修改指向为 baseQueue 的队首
...
function mountEffectImpl(fiberFlags, hookFlags, create, deps) {
  ...
  // 3. 创建 effect 链表(回调函数、依赖项), 挂载到 hook.memoizedState 上, effect 链表同时会保存在 fiber.updateQueue 中.
...
function dispatchAction(fiber, queue, action) {
  ...
  // 1. 创建update对象
  var eventTime = requestEventTime();
  var lane = requestUpdateLane(fiber); // Legacy 模式返回 SyncLane: 1
  var update = {
    lane: lane,
    action: action, // 更新内容
    eagerReducer: null,
   ...
   */
   /*
      ...
      但是当fiber上不存在update(初始化更新时)，则调用阶段创建的update为该hook上第一个update，在声明阶段计算state时也只依赖于该update，完全不需要进入声明阶段再计算state.
    ...
    if (fiber.lanes === NoLanes && (alternate === null || alternate.lanes === NoLanes)) {
      ...
      if (lastRenderedReducer !== null) {
        ...
        try {
          ...
          // 储存最新的 state, 后续更新触发 updateReducer 时, 直接从 update.eagerState 获取最新值, 无需重新计算.
...
var didReceiveUpdate = false; // 本次更新 fiber 是否变化
...
function reconcileChildren(current, workInProgress, nextChildren, renderLanes) {
  if (current === null) {
    // 初始化时除了 rootFiber 外, 其余的子 fiber 还未创建都为 null
    // If this is a fresh new component that hasn't been rendered yet, we
    // won't update its child set by applying minimal side-effects. Instead,
    // we will add them all to the child before it gets rendered. That means
    // we can optimize this reconciliation pass by not tracking side-effects.
    // 创建子节点 fiber 并挂载到 workInProgress.child 下
    ...
    // let's throw it out.
    // 创建子节点 fiber 并挂载到 workInProgress.child 下(初始化时, workInProgress 是 rootFiber)
...
function updateFunctionComponent(current, workInProgress, Component, nextProps, renderLanes) {
  ...
  // 需要更新, 则走下面逻辑
  workInProgress.flags |= PerformedWork;
  // 根据新的 reactElement 对象和 current fiber 节点做对比并重新创建新的 fiber 节点
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress.child;
}
...
function updateHostComponent(current, workInProgress, renderLanes) {
  ...
  // fiber 增加Ref effectTag
  markRef(current, workInProgress);
  // 创建子节点 fiber, 储存到 workInProgress.child
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress.child;
}
...
function mountIndeterminateComponent(_current, workInProgress, Component, renderLanes) {
  ...
  // Eventually we'll delete this branch altogether.
   typeof value === 'object' && value !== null && typeof value.render === 'function' && value.$$typeof === undefined) {
    ...
    // 根据 value(ReactElement 树)构造 fiber 树
    ...
    // 返回生成的 fiber, 回到 performUnitOfWork 函数将 fiber 作为当前的工作单元(workInProgress),
    // 循环这个过程直到 workInProgress 为 null, 这个完成后会进入提交阶段 commitRoot
    return workInProgress.child;
  }
}
...
function beginWork(current, workInProgress, renderLanes) {
  ...
  if (current !== null) {
    ...
    if (oldProps !== newProps || hasContextChanged() || ( // Force a re-render if the implementation changed due to hot reload:
     workInProgress.type !== current.type )) {
      ...
      // 本次更新中 fiber 节点是否有变化, 取决于: 对比新旧 props 是否变化, context 是否变化, type 是否改变
      didReceiveUpdate = true;
    } else if (!includesSomeLane(renderLanes, updateLanes)) {
      // 当前更新是不包含需要进行的任务, 初始化不会进入
  ...
  switch (workInProgress.tag) {
    /*
      函数组件默认是 indeterminateComponent 类型, 在初始化后会修改为函数类型,
      函数组件也可以作为类组件使用: 返回 render 函数即可. 所以 react 定义了 IndeterminateComponent
      类型来在组件初始化时判断是函数组件还是类组件
    */
    // 处理函数/类组件
    case IndeterminateComponent:
      {
    ...
    case ClassComponent:
      {
        if (finishedWork.flags & Snapshot) {
          if (current !== null) {
            ...
            // 触发 getSnapshotBeforeUpdate 生命周期函数
      ...
      {
        {
          if (finishedWork.flags & Snapshot) {
            ...
            // 调用 clearContainer 清空了容器节点(即 div#root 这个 dom 节点)
  ...
  if (lastEffect !== null) {
    ...
    do {
      if ((effect.tag & tag) === tag) {
        ...
        effect.destroy = create(); // 回调函数的返回值即是销毁函数
  ...
  if (lastEffect !== null) {
    ...
    do {
      ...
      if ((tag & Passive$1) !== NoFlags$1 && (tag & HasEffect) !== NoFlags$1) {
        // push useEffect 的销毁函数并且加入调度
        enqueuePendingPassiveHookEffectUnmount(finishedWork, effect);
        // push useEffect的 回调函数并且加入调度
...
function commitPlacement(finishedWork) {
  ...
  // 获取Fiber节点的DOM兄弟节点(insertBefore 中使用)
  var before = getHostSibling(finishedWork); // We only have the top Fiber that was inserted but we need to recurse down its
  // children to find all the terminal nodes.

  // parentStateNode 是否是 rootFiber(fiber 非 dom 节点, 需要向上获取父级 dom 节点)
  if (isContainer) {
    // 向上查找父级节点再插入
...
var rootDoesHavePassiveEffects = false; // 是否有 useEffect 需要处理
...
function scheduleUpdateOnFiber(fiber, lane, eventTime) {
  ...
  if (lane === SyncLane) {
    ...
    (executionContext & (RenderContext | CommitContext)) === NoContext) {
      ...
      // 执行上下文为空(非可控更新任务: setTimeout, Promise 触发)主动调用 flushSyncCallbackQueue, 直接进入fiber树构造循环(无需再次等待scheduler调度)
      // 如果是在合成事件中触发, 会在 batchUpdates$1 方法中为 executionContext 添加批处理上下文, 因此不会进入
    ...
    // 无论是否初次更新, 都会注册调度任务, 经过`Scheduler`包的调度, 间接进行`fiber构造`
    ensureRootIsScheduled(root, eventTime);
    schedulePendingInteractions(root, lane);
...
function markUpdateLaneFromFiberToRoot(
  sourceFiber, // sourceFiber表示被更新的节点
  lane // lane表示update优先级
) {
...
function ensureRootIsScheduled(root, currentTime) {
  // 前半部分: 判断是否需要注册新的调度
  // callbackNode: 正在执行中的调度任务, 会在 commitRootImpl 中被重置为 null
  ...
  if (existingCallbackNode !== null) {
    ...
    // 节流: 本次任务优先级和 root 已经存在的优先级相同, 如连续多次执行setState, 则无需注册新 task (继续沿用上一个优先级相同的task), 直接退出调用.
    ...
    // 防抖: 本次优先级和 root 已经存在的优先级不同, 则取消正在调度的 task, 重新注册新 task.
    cancelCallback(existingCallbackNode);
...
// 由 batchedEventUpdates 触发
function batchedEventUpdates$1(fn, a) {
  // 储存当前执行上下文, executionContext 会在 discreteUpdates$1 节点被赋值
  var prevExecutionContext = executionContext;
  // 当前执行上下文添加事件处理上下文
  ...
  try {
    ...
    // 执行上下文为空(没有调度任务), 同步执行更新任务(performSyncWorkOnRoot)
    if (executionContext === NoContext) {
      // Flush the immediate callbacks that were scheduled during this batch
      resetRenderTimer();
      // 批量更新流程，没有更新状态下，那么直接执行更新任务
      flushSyncCallbackQueue();
    }
  }
}
// 由 discreteUpdates 触发
// fn: dispatchEvent
function discreteUpdates$1(fn, a, b, c, d) {
  // executionContext 初始化为 0
  var prevExecutionContext = executionContext;
  // 将当前执行上下文置为 4 (离散合成事件)
  ...
  {
    try {
      // 以 UserBlockingPriority 优先级执行 dispatchEvent
...
function prepareFreshStack(root, lanes) {
  ...
  // 创建 workInProgress, root.current.alternate
...
function renderRootSync(root, lanes) {
  ...
  if (workInProgressRoot !== root || workInProgressRootRenderLanes !== lanes) {
    // 刷新栈帧, legacy模式下都会进入(创建新的 workInProgress)
...
function workLoopSync() {
  ...
  /*
    1.可以明显的看出, 整个 fiber 树构造是一个深度优先遍历(可参考 React 算法之深度优先遍历), 其中有 2
      个重要的变量 workInProgress 和 current (可参考前文fiber 树构造(基础准备)中介绍的双缓冲技术):
        - workInProgress 和 current 都视为指针
        - workInProgress 指向当前正在构造的 fiber 节点
        - current = workInProgress.alternate(即 fiber.alternate), 指向当前页面正在使用的fiber节点.
          初次构造时, 页面还未渲染, 此时current = null.

    2.在深度优先遍历中, 每个fiber节点都会经历 2 个阶段:
        - 探寻阶段 beginWork
        - 回溯阶段 completeWork1
...
function completeUnitOfWork(unitOfWork) {
  ...
  do {
    ...
    if ((completedWork.flags & Incomplete) === NoFlags) {
      ...
      // 如果父级 fiber flags 包含 Incomplete 代表父级的子 fiber 没有全部完成 beginWork 与 completeWork 整个流程
      if (returnFiber !== null && // Do not append effects to parents if a sibling failed to complete
      (returnFiber.flags & Incomplete) === NoFlags) {
        // 进入则说明父级的子 fiber 全部完成了 beginWork 与 completeWork 整个流程
        // Append all the effects of the subtree and this fiber onto the effect
        // list of the parent. The completion order of the children affects the
        // side-effect order.
        // 2. 收集当前Fiber节点以及其子树的副作用effects
        // 2.1 把子节点已有的的副作用队列添加到父节点上(子节点可能没有副作用队列)
        ...
        // 2.2 根据 flags 判断当前 fiber 是否有副作用, 如果有就将其添加到副作用列表的最后.
        if (flags > PerformedWork) {
          if (returnFiber.lastEffect !== null) {
            // 如果父级的副作用列表有 lastEffect fiber, 放到它的的后面(最晚执行 completeWork 的 fiber 放到 最早执行 completeWork 的 fiber 的后面)
            returnFiber.lastEffect.nextEffect = completedWork;
          } else {
            // 父级的没有 lastEffect fiber, 说明是副作用列表还是空, 先放到 firstEffect 上,
            // 此操作会使副作用列表的 firstEffect 永远指向最早处理的 fiber(最早执行 completeWork 的 fiber)
            returnFiber.firstEffect = completedWork;
          }
          // 将父级的 lastEffect 指向当前 fiber, 此操作会使副作用列表的 lastEffect 永远指向最晚处理的 fiber(最晚执行 completeWork 的 fiber)
...
function commitRootImpl(root, renderPriorityLevel) {
  do {
    ...
    // 调用 useEffect 与其他同步任务。由于这些任务可能触发新的渲染，所以这里要一直遍历执行直到没有任务
    // 在开始本次 commit 阶段之前, 如果有未执行的 useEffect, 则执行这些 useEffect 的销毁和回调函数
    // 处理之前遗留的 useEffect
    flushPassiveEffects();
  ...
  // 2.检测同步任务, 如果有则主动调用 flushSyncCallbackQueue (无需再次等待scheduler调度), 再次进入fiber树构造循环
  // 执行同步任务，这样同步任务不需要等到下次事件循环再执行
  // 比如在 componentDidMount 中执行 setState 创建的更新会在这里被同步执行或 useLayoutEffect 的回调函数
  flushSyncCallbackQueue();
  ...
  while (nextEffect !== null) {
    ...
    if ((flags & Passive) !== NoFlags) {
      ...
      if (!rootDoesHavePassiveEffects) {
        ...
        // scheduleCallback 调度 useEffect(在 mutation 之后才会异步处理)
        scheduleCallback(NormalPriority$1, function () {
          flushPassiveEffects(); // 执行 useEffect 的回调函数
          return null;
        });
      }
    }
...
// dom 变更, 界面得到更新, 主要处理副作用队列中带有 ContentReset, Ref, Placement, Update, Deletion, Hydrating 标记的 fiber 节点.
function commitMutationEffects(root, renderPriorityLevel) {
  ...
  while (nextEffect !== null) {
    ...
    if (flags & Ref) {
      ...
      if (current !== null) {
        // 解绑 ref, 在 commitRoot 的第三阶段(dom变更后), 再重新赋值(在 mount 阶段不会进入)
...
function commitLayoutEffects(root, committedLanes) {
  ...
  while (nextEffect !== null) {
    ...
    // 1.处理 Update 和 Callback 标记(调用生命周期钩子和hook, 执行顺序: 先设置 ref, 再执行回调)
...
function createWorkInProgress(current, pendingProps) {
  ...
  // 初始化或者第二次更新时: current = rootFiber, rootFiber.alternate = null
  if (workInProgress === null) {
    ...
    // 初始化时创建 rootFiber 作为 workInProgress, workInProgress 是正在构建中的 fiber 树
...
function updateContainer(element, container, parentComponent, callback) {
  ...
  update.payload = {
    element: element // 储存作为渲染的 reactElement 对象(从 App 开始)
  };
  ...
  // 最初的ReactElement对象<App/>被挂载到HostRootFiber.updateQueue.shared.pending.payload.element中,
  // 后文fiber树构造过程中会再次变动.
